...
describe provider_class, if: valid_lens? do
  ...
  context 'with full file' do
    ...
    it 'lists instances' do
      ...
      expect(inst[7]).to eq({ path_regex: :false, methods: %w[find save], environments: [], authenticated: 'any', allow: ['*'], allow_ip: [], ensure: :present, name: 'Auth rule for /certificate_request', path: '/certificate_request' })
    end
    ...
    describe 'when creating settings' do
      ...
      it 'creates an entry with a regex path' do
        apply!(Puppet::Type.type(:puppet_auth).new(
                 name: 'Matching ^/foo/([^/]+)$',
                 path: '^/foo/([^/]+)$',
                 path_regex: 'true',
